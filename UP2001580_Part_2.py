# -*- coding: utf-8 -*-
"""
Created on Sat Dec  9 16:44:51 2023
...
class Full_NN(object):
    ...
    def train_neural_network(self, x, target, epochs, lr):   #training the network. The x is an array, the target is an array the epochs is a number and the lr is a number. 
...
if __name__ == "__main__":
    ...
    # For the OR problem
    training_inputs_or = np.array([[random()/2 for _ in range(2)] for _ in range(1000)])
    targets_or = np.array([[i[0] or i[1]] for i in training_inputs_or])

    nn_or = Full_NN(2, [5, 5], 1)
    nn_or.train_neural_network(training_inputs_or, targets_or, 10, 0.1)

    # Testing the OR problem
    input_or = np.array([[0, 0], [0, 1], [1, 0], [1, 1]])
    target_or = np.array([[0], [1], [1], [1]])

    nn_output_or = nn_or.FF(input_or)

    print("=============== Testing the OR Problem ===============")
    print("Test input is ", input_or)
    print()
    print("Target output is ", target_or)
    print()
    print("Neural Network actual output is ", nn_output_or, "with an error of ", target_or - nn_output_or)
    print("=======================================================")

    # For the XOR problem
    training_inputs_xor = np.array([[random()/2 for _ in range(2)] for _ in range(1000)])
    targets_xor = np.array([[i[0] != i[1]] for i in training_inputs_xor])

    nn_xor = Full_NN(2, [5, 5], 1)
    nn_xor.train_neural_network(training_inputs_xor, targets_xor, 10, 0.1)

    # Testing the XOR problem
    input_xor = np.array([[0, 0], [0, 1], [1, 0], [1, 1]])
    target_xor = np.array([[0], [1], [1], [0]])  # XOR target values

    nn_output_xor = nn_xor.FF(input_xor)

    print("=============== Testing the XOR Problem ===============")
    print("Test input is ", input_xor)
    print()
    print("Target output is ", target_xor)
    print()
    print("Neural Network actual output is ", nn_output_xor, "with an error of ", target_xor - nn_output_xor)
    print("=======================================================")
